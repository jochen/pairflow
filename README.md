...
> **Status:** All four planned tiers implemented and tested. Twenty-six MCP tools live, 257 tests green on Python 3.11/3.12/3.13.
>
> *Tier 1 — flow surgery:* `nr_list_tabs`, `nr_list_nodes`, `nr_get_node` (with opt-in `include_sources` reverse-wires lookup), `nr_search_flows` (global string/regex search across every flow field), `nr_list_dangling` (orphan link-in / link-out enumeration), `nr_add_node`, `nr_update_node`, `nr_delete_node`, `nr_wire`, `nr_unwire`, `nr_validate_function`, `nr_run_function` — with atomic writes, automatic timestamped backups, optimistic mtime locking, pre-write JS syntax validation for function nodes, and sandboxed execution of function bodies (Node.js subprocess; captures `node.send`/`warn`/`error` and async failures inside `setTimeout`/`Promise` callbacks). Plus credential-store tools `nr_set_credentials`, `nr_list_credentials`, `nr_delete_credentials` — direct AES-256-CTR writes to Node-RED's encrypted credential file (the only reliable way to set credentials outside the editor; the `credentialSecret` is read transiently from `settings.js` and never stored).
>
