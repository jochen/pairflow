...
>
> *Tier 1 — flow surgery:* `nr_list_tabs`, `nr_list_nodes`, `nr_get_node` (with opt-in `include_sources` reverse-wires lookup), `nr_search_flows` (global string/regex search across every flow field), `nr_list_dangling` (orphan link-in / link-out enumeration), `nr_add_node`, `nr_update_node`, `nr_delete_node`, `nr_wire`, `nr_unwire`, `nr_validate_function`, `nr_run_function` — with atomic writes, automatic timestamped backups, optimistic mtime locking, pre-write JS syntax validation for function nodes, and sandboxed execution of function bodies (Node.js subprocess; captures `node.send`/`warn`/`error` and async failures inside `setTimeout`/`Promise` callbacks).
>
